...
1. Before changing spell code, read `docs/spells/README.md` and the spell-specific document at `docs/spells/<spell_id>.md`.
2. Treat each spell separately. Every spell must have its own document named with its registry path in lowercase snake case, for example `docs/spells/wings_of_tempest.md`.
...
8. `docs/all_spells.md` may remain a summary or index, but it does not replace the required per-spell document in `docs/spells/`.
