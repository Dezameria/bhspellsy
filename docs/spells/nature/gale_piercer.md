...
- Particle visual effects:
  ...
  - **Charging Wind (< 200 ticks)**: Swirling wind vortex gathering inward onto the arrow entity from a 0.45-0.90 block radius. Combines tangential angular swirl with inward velocity, using `ParticleTypes.CLOUD`, pure white wind dust (`#FFFFFF`), soft rose dust (`#F5B0C0`), and delicate plum blossom petals (`ParticleRegistry.RED_PLUM`). Two flowing wind ribbons trace a double helix around the arrow shaft with periodic whistling wind audio (`SoundEvents.ELYTRA_FLYING`).
  - **Full Charge Transition (= 200 ticks)**: Instant fiery ignition on the arrow. Features explosive ignition audio (`SoundEvents.FIRECHARGE_USE` + `SoundEvents.BLAZE_SHOOT`), arrow-centered `FLASH`, dense burst of `FLAME`, `LAVA` sparks, and peach-orange, golden-yellow, and vermilion-crimson dust.
  - **Full Charge Hold (> 200 ticks)**: Arrow is completely enveloped in blazing fire. Two spiraling fire ribbons wrap around the shaft (`FLAME` + peach/rose dust), the arrowhead tip burns with intense flame and popping lava sparks, accompanied by subtle campfire crackle audio.
