...
@OnlyIn(Dist.CLIENT)
public class GoldChainRenderer extends EntityRenderer<GoldChain> {
    public static final ResourceLocation CHAIN_TEXTURE = new ResourceLocation(IronSpellMore.MODID, "textures/entity/gold_chain.png");

    public GoldChainRenderer(EntityRendererProvider.Context context) {
        super(context);
        this.shadowRadius = 0f;
    }

    @Override
    public void render(GoldChain entity, float entityYaw, float partialTick, PoseStack poseStack,
        ...
        Vec3 lastWorldpos = new Vec3(Mth.lerp(partialTick, last.xo, last.getX()),
                Mth.lerp(partialTick, last.yo, last.getY()), Mth.lerp(partialTick, last.zo, last.getZ()));
        Vec3 targetPos = lastWorldpos.subtract(entity.position());
        float warmup = Mth.clamp((entity.warmup + partialTick) / GoldChain.VISUAL_WARMUP_TIME, 0, 1f);
        renderChainBetween(Vec3.ZERO, targetPos, poseStack, bufferSource, warmup);

        super.render(entity, entityYaw, partialTick, poseStack, bufferSource, packedLight);
    }

    public static void renderChainBetween(Vec3 start, Vec3 to, PoseStack poseStack,
    ...
    @Override
    public ResourceLocation getTextureLocation(GoldChain entity) {
