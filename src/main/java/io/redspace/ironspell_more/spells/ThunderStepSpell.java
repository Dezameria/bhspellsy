...
public class ThunderStepSpell extends AbstractSpell {
    ...
    public void onCast(Level level, int spellLevel, LivingEntity entity, CastSource castSource,
        ...
        for (int i = 0; i < 7; i++) {
            Vec3 random1 = Utils.getRandomVec3(0.5f).multiply(entity.getBbWidth(), entity.getBbHeight(),
                    entity.getBbWidth());
            Vec3 random2 = Utils.getRandomVec3(0.8f).multiply(entity.getBbWidth(), entity.getBbHeight(),
                    entity.getBbWidth());
            float yOffset = i / 7f * entity.getBbHeight();
            Vec3 midpoint = entity.position().add(travel.scale(0.5f)).add(random2);
            ((ServerLevel) level).sendParticles(
                    new ZapParticleOption(random1.add(entity.getX(), entity.getY() + yOffset, entity.getZ())),
                    midpoint.x, midpoint.y, midpoint.z, 1, 0, 0, 0, 0);
            ((ServerLevel) level).sendParticles(
                    new ZapParticleOption(random1.scale(-1f).add(dest.x, dest.y + yOffset, dest.z)), midpoint.x,
                    midpoint.y, midpoint.z, 1, 0, 0, 0, 0);
        }
    ...
    private void zapEntitiesBetween(LivingEntity caster, int spellLevel, Vec3 blockEnd) {
        ...
        for (Entity target : entities) {
            ...
            if (Utils.checkEntityIntersecting(target, start, end, 1f).getType() != HitResult.Type.MISS
                    || Utils.checkEntityIntersecting(target, start.subtract(height), end.subtract(height), 1f)
